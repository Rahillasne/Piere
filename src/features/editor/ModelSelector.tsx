    ...
    <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          variant="ghost"
          className={cn(
            'flex h-8 w-auto items-center gap-1.5 rounded-lg px-3 text-sm transition-all duration-200 hover:border-[#333333] hover:bg-pierre-neutral-800',
            focused
              ? 'text-white hover:text-white'
              : 'text-pierre-text-secondary hover:text-pierre-text-primary',
            isDropdownOpen &&
              (focused
                ? 'bg-pierre-neutral-800 text-white'
                : 'bg-pierre-neutral-800 text-pierre-text-primary'),
            className,
          )}
          disabled={!!disabled}
        >
      ...
      <DropdownMenuContent
        className="flex w-64 flex-col gap-1 rounded-lg bg-pierre-neutral-700 p-1"
        align="end"
      ...
      >
        {models.map((model) => (
          <DropdownMenuItem
            key={model.id}
            className={cn(
              'cursor-pointer rounded-md bg-pierre-neutral-700 px-4 py-3 transition-colors duration-150 focus:bg-pierre-bg-secondary-dark',
              selectedModel === model.id && 'bg-pierre-neutral-800',
            )}
            onClick={(event) => {
            ...
            <div className="flex-1">
              <div className="flex items-center">
                <span
                  className={cn(
                    'font-medium',
                    focused ? 'text-white' : 'text-pierre-text-primary',
                  )}
                >
                  {model.name}
                </span>
