...
export function AssistantMessage({
  ...
  const model = message.content.model ?? 'pierre';
    ...
    <div className="flex justify-start">
      {message.role === 'assistant' && (
        <div className="mr-2 mt-1">
          <Avatar className="h-9 w-9 border border-pierre-neutral-700 bg-pierre-neutral-950">
      ...
      <div className="w-[80%] rounded-lg bg-pierre-neutral-800">
        <div className="flex flex-col gap-3 p-3 text-sm text-pierre-text-primary">
                  ...
                  <div className="flex w-full flex-col gap-2">
                    {message.content.toolCalls.map((toolCall) => (
                      <div
                        ...
                        className="flex h-10 w-full items-center justify-between overflow-hidden rounded-md bg-pierre-neutral-950 px-3 hover:bg-pierre-neutral-900"
                      >
          ...
          {(message.siblings.length > 1 ||
            ...
            <div className="flex items-center gap-1">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        restoreMessage({
                          ...
                          currentLeafId: conversation.current_message_leaf_id,
                        })
                      }
                      disabled={isLoading}
                      className="h-6 w-6 rounded-lg p-0"
                    >
                      <History className="h-3 w-3 p-0 text-pierre-neutral-100" />
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
                    <span>Restore</span>
                ...
                <div className="flex items-center">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      ...
                      >
                        <RefreshCw className="h-3 w-3 p-0 text-pierre-neutral-100" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                      <span>Retry</span>
              ...
              {message.siblings.length > 1 && (
                <div className="flex h-6 items-center gap-0.5 rounded-lg border border-pierre-neutral-700 bg-pierre-bg-secondary-dark">
                  <Button
...
function ObjectButton({
  ...
  return (
    <Button
      ...
      className={cn(
        'group relative bg-black p-2 hover:bg-pierre-bg-dark',
    ...
    >
      <div className="flex w-full items-center justify-between border-gray-200/20 pr-16 dark:border-gray-700">
        <div className="flex items-center space-x-2">
          <Box className="h-4 w-4 text-pierre-text-primary" />
          <span className="font-medium text-pierre-text-primary">{title}</span>
        </div>
        <span
          className={cn(
            'absolute right-2 flex h-6 items-center overflow-hidden rounded-md border border-pierre-neutral-700 bg-pierre-bg-secondary-dark px-1 text-xs transition-all duration-100 ease-in-out hover:bg-black',
            isHovered
              ? 'w-14 text-pierre-text-primary'
              : `w-${6 + (currentVersion.toString().length - 1)} text-pierre-neutral-300`,
          )}
        >
          {isHovered ? (
            <div className="flex items-center gap-1">
  ...
  if (availableModels.length === 0) {
    return (
      <Button
        ...
        disabled={true}
        className={cn(
          'h-6 w-fit gap-1 rounded-lg px-2 text-xs text-pierre-text-primary opacity-50',
          className,
        )}
      >
    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          disabled={disabled}
          className={cn(
            'h-6 w-fit gap-1 rounded-lg px-2 text-xs text-pierre-text-primary',
            isOpen && 'bg-pierre-neutral-800',
            className,
          )}
        >
          <span>{selectedModelConfig.name}</span>
      ...
      <DropdownMenuContent
        className="w-48 rounded-lg border border-pierre-neutral-700 bg-pierre-neutral-800 p-1"
        align="start"
      >
        {availableModels.map((model) => (
          <DropdownMenuItem
            key={model.id}
            className={cn(
              'cursor-pointer rounded-md bg-pierre-neutral-800 px-2 py-1.5 text-xs text-pierre-text-primary hover:bg-pierre-neutral-700 focus:bg-pierre-bg-secondary-dark',
            )}
            onClick={() => {
              if (onRetry) {
                onRetry(model.id);
