...
function TextAreaChat({
  onSubmit,
  placeholder = 'What can Pierre help you build today?',
  disabled = false,
  model,
  setModel,
  conversation,
  ...
  const { mutateAsync: uploadImageAsync } = useMutation({
    mutationFn: async ({ file, id }: { file: File; id: string }) => {
      // Ensure we have a valid user_id before uploading
      ...
      newItems.map(async (file) => {
        if (file.type.includes('image')) {
          ...
          if (!VALID_IMAGE_FORMATS.includes(file.type)) {
            ...
            return { type: 'image' as const, file: null };
          }
          ...
          const validFile = await new Promise<File | null>((resolve) => {
            const img = new Image();
          ...
          return { type: 'image' as const, file: validFile };
        }

        return { type: 'unknown' as const, file: null };
      }),
    );

    // Separate valid images
    const validImages = processedFiles
      .filter((item) => item.type === 'image' && item.file !== null)
  ...
  useEffect(() => {
    ...
    if (totalFiles > 1 && model !== 'metroboomin') {
      setModel('metroboomin');
    }
  }, [images.length, setModel]);
  ...
  const handleImageRemoved = async (image: MessageItem) => {
    if (!image.isUploading) {
      ...
      if (image.source === 'upload' && conversation.user_id) {
        try {
  ...
  return (
    ...
    >
          ...
          disabled
            ...
            : !isDragging && images.length === 0
              ...
              : isDragging
                ...
                : images.length > 0
                  ? 'h-20 border-pierre-neutral-700 bg-pierre-neutral-950 opacity-100'
                  : 'h-0 border-transparent bg-transparent opacity-0',
        )}
        onDragEnter={(event) => {
          if (isDragging) {
      ...
      >
        {!disabled && (
          <>
            {/* Case 1: Dragging, and items are ALREADY present -> Show "Add more files" prompt */}
            {isDragging && images.length > 0 ? (
              <div
                className={cn(
            ...
            ) : /* Case 2: No items -> Show original "Drop images here" logic */
            images.length === 0 ? (
              <div
                className={cn(
                  'flex h-full w-full flex-row items-center justify-center gap-2',
                    ...
                    {images.map((image) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className={cn(
                            'absolute right-[-0.50rem] top-[-0.50rem] rounded-full border border-pierre-neutral-500 bg-pierre-neutral-500 text-white transition-colors duration-200 hover:border-pierre-neutral-700 hover:bg-pierre-neutral-700',
      ...
      <div
        ...
        className={cn(
          ...
          isFocused
            ? 'border-pierre-blue shadow-[inset_0px_0px_8px_0px_rgba(0,0,0,0.08)]'
            : 'border-pierre-neutral-700 shadow-[inset_0px_0px_8px_0px_rgba(0,0,0,0.08)] hover:border-pierre-neutral-400',
          'bg-pierre-background-2 transition-all duration-300',
        )}
        onDragEnter={(event) => {
          if (isDragging) {
            event.preventDefault();
        ...
        <div className="flex items-center justify-between border-t border-[#2a2a2a] p-3">
          <div className="flex items-center gap-3">
            ...
            >
              <Button
                variant="outline"
                className="flex h-8 w-8 items-center gap-2 rounded-lg border border-[#2a2a2a] bg-pierre-background-2 p-0 text-sm text-pierre-text-secondary hover:bg-pierre-bg-secondary-dark"
                onClick={(e) => {
                  e.stopPropagation();
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={cn(
                'flex h-8 w-8 transform items-center justify-center rounded-lg bg-pierre-neutral-700 p-1 text-white transition-all duration-300 hover:scale-105 hover:bg-pierre-blue/90 disabled:opacity-50 disabled:hover:scale-100 disabled:hover:bg-pierre-blue',
